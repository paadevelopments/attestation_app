...
public class MainActivity extends FlutterActivity {
            ...
            switch (call.method) {
                case "getSecurityReport":
                    try {
                        ...
                        Map<String, Object> report = buildSecurityReport(nonce);
                        result.success(report);
                    } catch (Exception e) {
                        Log.e(TAG, "Security report generation failed", e);
                        result.error(
                                "ATTESTATION_ERROR",
                                e.getMessage(),
                                null
    ...
    private Map<String, Object> buildSecurityReport(String nonce) {
        ...
        report.put("isSELinuxEnforced", isSELinuxEnforced());
        report.put("apkSignatureHash", getApkSignatureHash());
    ...
    public boolean isBootloaderLocked(List<byte[]> certBytesList) {
        try {
            ...
            ASN1Boolean deviceLocked = ASN1Boolean.getInstance(rootOfTrust.getObjectAt(1));
            ASN1Enumerated verifiedBootState = ASN1Enumerated.getInstance(rootOfTrust.getObjectAt(2));
            /*
             * VERIFIED = 0
             */
            return deviceLocked.isTrue() && verifiedBootState.getValue().intValue() == 0;
        } catch (Exception e) {
    ...
    public String getVerifiedBootState(List<byte[]> certBytesList) {
        try {
            ...
            ASN1Enumerated verifiedBootState = ASN1Enumerated.getInstance(rootOfTrust.getObjectAt(2));
            int state = verifiedBootState.getValue().intValue();
            return switch (state) {
                case 0 -> "VERIFIED";
                case 1 -> "SELF_SIGNED";
                case 2 -> "UNVERIFIED";
        ...
        try {
            ...
            for (byte[] certBytes : certBytesList) {
                try {
                     ...
                     */
                    int[] probableIndexes = {6, 7};
                    for (int targetIndex : probableIndexes) {
                        if (targetIndex >= keyDescription.size()) {
                            continue;
                        }
                        Object current = keyDescription.getObjectAt(targetIndex);
                        if (!(current instanceof ASN1Sequence)) {
                            continue;
                        }
                        ASN1Sequence teeEnforced = ASN1Sequence.getInstance(current);
                        for (int i = 0; i < teeEnforced.size(); i++) {
                            Object entry = teeEnforced.getObjectAt(i);
                    ...
                    Log.w(TAG, "Skipping malformed certificate", inner);
                }
            }
        } catch (Exception e) {
            Log.e(TAG, "RootOfTrust parse failed", e);
        }
        return null;
    }
    ...
    public boolean isStrongBoxSupported() {
        ...
        return getPackageManager().hasSystemFeature(PackageManager.FEATURE_STRONGBOX_KEYSTORE);
    }
    ...
    public String getApkSignatureHash() {
        try {
            ...
            Signature signature = Objects.requireNonNull(packageInfo.signatures)[0];
