...
import "package:attestation_app/attestation_api.dart";
import "package:attestation_app/constants.dart";
...
class _MyHomePageState extends State<MyHomePage> {
  ...
  final AttestationApi api = AttestationApi(Constants.baseUrl);
  ...
  Future<void> _runAttestation() async {
    ...
    try {
      final report = await platform.invokeMethod("getSecurityReport", {
        "nonce": _nonce,
      });
  ...
  Map<dynamic, dynamic> _truncateCertificateChain(dynamic rawReport) {
    final Map<dynamic, dynamic> reportMap = Map<dynamic, dynamic>.from(
      rawReport as Map,
    );
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(title: const Text("PCI MPoC Security Check")),
        ...
        child: Column(
          ...
          children: [
            ...
            if (_localReport != null)
              _infoCard("Device", _localReport.toString()),
            if (_serverResult != null)
              _infoCard("Backend Result", _serverResult.toString()),
          ],
        ),
      ),
    );
